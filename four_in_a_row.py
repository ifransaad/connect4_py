...
###############################################################################
    ...
    def max_value(player, board, depth_limit):
        ...
        #score to minimum and best_move to default first move
        score = -math.inf
        best_move = moves[0]
...
###############################################################################
    ...
    def max_value(player, board, depth_limit, , alpha, beta):
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        #tuples containing column, and board_copy (for all valid moves)
        moves = get_child_boards(player, board)
        ...
        for move in moves:
            ...
            if max_score > score :
                best_move = column
                score = max_score
            #Pruning, if alpha > beta then min player will not choose any remaining paths
...
###############################################################################
    ...
    #The chance player (did not change function name because not specified in project requirements)
    
    def min_value(player, board, depth_limit):
        next_player = board.PLAYER2 if player == board.PLAYER1 else board.PLAYER1
        #tuples containing column, and board_copy (for all valid moves)
        moves = get_child_boards(player, board)
        #score to 0 (neutral) and probability based on number of moves
        average_score = 0
        ...
        for move in moves:
            ...
            #recursively call value as next player, calculate the average score with all moves being equally weighted probability.
